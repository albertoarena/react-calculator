...
export default class Calculator {
  ...
  _applyImmediately(item) {
    this._addNumber();
    this.output.push(
      new BaseNumber(item.run(0, this._getNextValue()), item.toFullString()),
    );
  }
  ...
  getResult() {
    return {
      processed: this.processed,
      total: this._lastException
        ? "Error"
        : this.queue.length
          ? _.last(this.queue).getValue() + ""
          : null,
      next: this._getValue(),
      // operation: null,
  ...
  _getHistory() {
    ...
    return (
      history
        .join(" ")
        .replace(
          new RegExp("[\\s]{1}" + Constants.getNoSpaceMarker(), "gi"),
          "",
        )
        .replace(
          new RegExp(Constants.getNoSpaceMarker() + "[\\s]{1}", "gi"),
          "",
        )
